package com.ttqeducation.teacher.network;
...
import com.ttqeducation.teacher.activitys.message.GroupChatActivity;
import com.ttqeducation.teacher.activitys.message.OneChatActivity;
import com.ttqeducation.teacher.activitys.notice.NoticeDetailActivity;
...
//		deviceID = telephonyManager.getDeviceId();
		...
		new Thread(new Runnable() {			
			...
			public void run() {
				// TODO Auto-generated method stub
//				socketClient = new SocketClient("192.168.137.130", 9050);	// hxy
//				socketClient = new SocketClient("192.168.137.123", 8081);	// yxj 115.29.161.214
//				socketClient = new SocketClient("115.29.161.214", 9050);	// 云平台；
	...
	private class SocketClient{
		...
		public void start(){
			try {
				...
				// 从本地获取学生ID；
				SharedPreferences pre = getSharedPreferences("TTQAndroid", MODE_PRIVATE);
				String execTeacherID = pre.getString("execTeacherID", "");
				writer = new DataOutputStream(socket.getOutputStream());
				reader = new DataInputStream(socket.getInputStream());
				String sendInfoString = execTeacherID.trim()+","+androidId+","+1;
				...
				if(isCloseSocketNormal){		// 是正常关闭socket的；
					...
					closeSocketAndHeart();		// 当出现异常，需要关闭socket；
		...
		public void closeSocket(){
				try {
					...
					// TODO Auto-generated catch block
					Log.i("lvjie", "socket关闭连接出错...");
					e.printStackTrace();
				}
				Log.i("lvjie", "closeSocket()-->正常关闭连接...");
		}
	...
	private void showReceiveMsgInView(String[] revieve){
		...
		case 4:		// 表示一对一聊天；
			...
			break;
		case 5:		// 表示班级群聊；
			exeGroupChatInfo(messageID, messageContent, messageTitle, revieve[4], revieve[5]);
			break;
	...
	private void showNotify(Intent intent,String ticker,String title,String content) {
		...
		Notification.Builder builder = new Builder(PushService.this)
				.setTicker(ticker).setSmallIcon(com.ttqeducation.teacher.R.drawable.ic_launcher)
	...
	private void exeOneChatInfo(int chatID, String chatInfo, String chatTilte, String chatingID){
		...
		if(TeacherInfo.getInstance().currentView == 1 && TeacherInfo.getInstance().chatingID.equals(chatingID)){		
			getNoticeContentByWS(1, chatID, TeacherInfo.getInstance().execTeacherID, chatingID);
		}else {
			String parentName = chatTilte.split("来自：")[1];
			Intent chatIntent = new Intent(PushService.this, OneChatActivity.class);
			chatIntent.putExtra("viewInitFrom", 2);
			chatIntent.putExtra("parentID", chatingID);
			chatIntent.putExtra("parentName", parentName);
			chatIntent.putExtra("parentImgID", R.drawable.parent12);
			Log.i("lvjie", "从中间件启动的界面-->parentName="+parentName+"  chatingID="+chatingID);
			showNotify(chatIntent, chatInfo, chatTilte, chatInfo);
		}
	}
	
	// 处理群聊发送过来的消息；
	private void exeGroupChatInfo(int chatID, String chatInfo, String chatTilte, String chatingID, String publisherID){
		// 表示当前用户正处在群聊界面且表示发送过来的消息者正好是当前群聊班级；
		if(TeacherInfo.getInstance().currentView == 2 && TeacherInfo.getInstance().chatingID.equals(chatingID)){
			Log.i("lvjie", "班级群聊消息，处在当前界面，直接去更新界面...");
			getNoticeContentByWS(2, chatID, TeacherInfo.getInstance().execTeacherID, chatingID);
		}else {
			String className = chatTilte.split("来自：")[1];
			Intent intentSystemNotice = new Intent(PushService.this, GroupChatActivity.class);
			intentSystemNotice.putExtra("viewInitFrom", 2);
			intentSystemNotice.putExtra("classID", chatingID);
			intentSystemNotice.putExtra("className", className);
			Log.i("lvjie", "从中间件启动的界面-->chatingID="+chatingID+"  className="+className);
			showNotify(intentSystemNotice, chatInfo, chatTilte, chatInfo);
		}
	}
	...
	// 从服务端获取某一条通知详情；
	public void getNoticeContentByWS(final int type, int noticeID, String teacherID, final String chatingID){
		...
		new AsyncTask<Object, Object, DataTable>() {
			...
			protected DataTable doInBackground(Object... params) {
				...
				// 方法名
				String methodName = "APP_getPushNotification_detail_teacher";
				// 存放参数的map
				Map<String, String> paramsMap = new HashMap<String, String>();
				Log.i("lvjie", "messageID="+params[0].toString()+"  teacherID="+params[1].toString());
...
//							String noticeTitle = result.getCell(i, "title");
							...
							String publisherName = result.getCell(i, "publisherName").trim();
							String publishTime = time.replace("T", " ");
							Log.i("lvjie", "1-->publishTime="+publishTime);
//							publishTime = publishTime.substring(0, publishTime.length()-6);
							Log.i("lvjie", "publishTime="+publishTime);
							try {
							...
							if(type == 1){		// 表示的是一对一聊天；
								Intent intent = new Intent("mobile.onechat");		// 设置广播名称；
								intent.addCategory("mobile.action.onechat");		// 设置广播种类；
								intent.putExtra("chatInfo", noticeContent);				// 广播传递的值；
								intent.putExtra("chatTime", publishTime);				// 广播传递的值；
								sendBroadcast(intent);
							}else {		// 表示的是群聊；
								Intent intent = new Intent("mobile.groupchat");		// 设置广播名称；
								intent.addCategory("mobile.action.groupchat");		// 设置广播种类；
								intent.putExtra("chatInfo", noticeContent);				// 广播传递的值；
								intent.putExtra("chatTime", publishTime);				// 广播传递的值；
								intent.putExtra("publisherName", publisherName);
								intent.putExtra("publisherID", chatingID);
								intent.putExtra("personImgID", TeacherInfo.getInstance().getPersonPicID(chatingID));
								sendBroadcast(intent);
							}
