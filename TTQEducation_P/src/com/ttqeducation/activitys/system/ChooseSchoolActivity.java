...
public class ChooseSchoolActivity extends Activity {
	...
	private ImageView ivChooseSchool = null; // 上方背景图片，做屏幕适配
...
//		initView1();
	...
	public void initView() {
		// 动态设置背景图片大小
		ivChooseSchool = (ImageView)findViewById(R.id.ivChooseSchool);
		// 获取屏幕宽度
		DisplayMetrics metrics = getResources().getDisplayMetrics(); // 用来获取屏幕的分辨率；
		int screenWidthPX = metrics.widthPixels;
		LayoutParams ivChooseSchoolParams = (LayoutParams)ivChooseSchool.getLayoutParams();
		ivChooseSchoolParams.width = (int)((screenWidthPX / (float)1080) * 552);
		ivChooseSchoolParams.height = (int)((screenWidthPX / (float)1080) * 291);
		ivChooseSchool.setLayoutParams(ivChooseSchoolParams);
		...
		new AsyncTask<Object, Object, DataTable>() {
			...
			protected void onPostExecute(DataTable result) {
				...
				if (result != null) {					
					...
					// data.putString("SCHOOL_SERVICE_URL", result);
	...
	public void getWS(){
		new AsyncTask<Object, Object, DataTable>() {
			...
			protected DataTable doInBackground(Object... params) {
				...
				Log.i("lvjie", "1**************");
				// 方法名
				String methodName = "pub_webUser_select";
				// 存放参数的map
				Map<String, String> paramsMap = new HashMap<String, String>();
				paramsMap.put("webUserName", "TTQCManage");
				paramsMap.put("webPassword", "TTQCManage");
				paramsMap.put("roleID", "1001");
				// 获取数据
				GetDataByWS getDataTool = GetDataByWS.getInstance();
				getDataTool.setURL("http://192.168.137.1:6666/ManageService.asmx");
